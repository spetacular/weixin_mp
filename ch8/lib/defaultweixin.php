    ...
    private function goroute($data) {
        ...
        $posts = array(
            array(
                ...
                'url' => BASE_URL.'/route.php?user=' . $data->FromUserName,
            )
        );
        ...
        if (!empty($ret) && $ret['dinertime'] > time() - 1800) {
            $posts = array(
                array(
                    ...
                    'url' => BASE_URL.'/myreserve.php?user=' . $data->FromUserName,
                )
            );
        } else {
            $posts = array(
                array(
                    ...
                    'description' => '现在预约吧',
                    'picurl' => 'http://mmbiz.qpic.cn/mmbiz/XWia2Xj7RZ8nxGcFl47qJQjsm1iaqf3SquWeL0BzAficTdkxo2oeV9PvVqvcUUj14pE4oq5fAZx2s4TGsZIalZFCg/0',
                    'url' => BASE_URL.'/reserve.php?user=' . $data->FromUserName,
                )
            );
        }
    ...
    private function showqr($data, $type = 0) {
        ...
        $posts = array(
            array(
                ...
                'url' => BASE_URL.'/showqr.php?sceneid=' . $sceneid . '&user=' . $data->FromUserName,
            )
        );
        $this->outputNews($posts);
    }
